...
"""
Dataloaders and dataset utils(改)

为了使之可以作为web服务运行直接接受图片数据,主要修改了此文件(`utils/datasets.py`):
    `class LoadImages` 相关部分
如果需要正常使用还是得复制原yolo中其他文件依赖
"""
...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, img=None):
        self.source_img = False
        if isinstance(img, np.ndarray):
            images = [img]
            videos = []
            self.source_img = True
        else:
